...
const Settings = ({ tenantId, token }) => {
    ...
    const [settings, setSettings] = useState({
        ...
        whatsapp_shared_number: '971557357753', // Default shared WhatsApp number
    });
    ...
    const fetchSettings = async () => {
        try {
            ...
            setSettings({
                ...
                whatsapp_shared_number: data.contact_phone?.replace('+', '') || data.phone?.replace('+', '') || '971557357753',
            });
    ...
    const handleSave = async () => {
        try {
            ...
            Object.keys(settings).forEach(key => {
                // Skip whatsapp_shared_number (it gets mapped to contact_phone)
                if (key === 'whatsapp_shared_number') return;
            ...
            // Map whatsapp_shared_number to contact_phone for backend
            if (settings.whatsapp_shared_number) {
                updateData.contact_phone = '+' + settings.whatsapp_shared_number.replace(/^\+/, '');
            }
        ...
        <div className="space-y-6">
            ...
            <div className="flex gap-2 border-b border-white/10 pb-0">
                ...
                <button
                    ...
                    className={`px-4 py-2 font-medium transition-colors border-b-2 ${activeTab === 'bots'
                        ? 'text-gold-500 border-gold-500'
                        : 'text-gray-400 border-transparent hover:text-white'
                        }`}
                >
                ...
                <button
                    ...
                    className={`px-4 py-2 font-medium transition-colors border-b-2 ${activeTab === 'schedule'
                        ? 'text-gold-500 border-gold-500'
                        : 'text-gray-400 border-transparent hover:text-white'
                        }`}
                >
                ...
                <>
                    ...
                    <div className="glass-card rounded-xl p-6 border-2 border-green-500/30">
                        ...
                        <div className="bg-navy-800 rounded-lg p-4 mb-4">
                            ...
                            {/* Editable WhatsApp Shared Number */}
                            <div className="mb-4">
                                <label className="text-gray-400 text-sm block mb-2 flex items-center gap-2">
                                    <Phone size={14} className="text-green-400" />
                                    WhatsApp Shared Number (editable)
                                </label>
                                <input
                                    type="text"
                                    value={settings.whatsapp_shared_number}
                                    onChange={(e) => setSettings({ ...settings, whatsapp_shared_number: e.target.value.replace(/[^0-9]/g, '') })}
                                    className="w-full bg-navy-900 text-white rounded-lg px-4 py-3 border border-green-500/50 focus:border-green-500 outline-none font-mono"
                                    placeholder="971557357753"
                                />
                                <p className="text-gray-500 text-xs mt-1">Enter phone number without + sign (e.g., 971557357753)</p>
                            </div>
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <code className="flex-1 bg-navy-900 text-green-400 px-3 py-2 rounded text-sm overflow-x-auto">
                                            https://wa.me/{settings.whatsapp_shared_number}?text=TENANT_{tenantId}
                                        </code>
                                        <button
                                            onClick={() => copyToClipboard(`https://wa.me/${settings.whatsapp_shared_number}?text=TENANT_${tenantId}`, 'deeplink')}
                                            className="p-2 bg-navy-900 rounded hover:bg-gold-500 hover:text-navy-900 transition-colors"
                                        >
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <code className="flex-1 bg-navy-900 text-green-400 px-3 py-2 rounded text-sm overflow-x-auto">
                                            https://api.qrserver.com/v1/create-qr-code/?data=https://wa.me/{settings.whatsapp_shared_number}?text=TENANT_{tenantId}
                                        </code>
                                        <a
                                            href={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=https://wa.me/${settings.whatsapp_shared_number}?text=TENANT_${tenantId}`}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="p-2 bg-navy-900 rounded hover:bg-gold-500 hover:text-navy-900 transition-colors"
